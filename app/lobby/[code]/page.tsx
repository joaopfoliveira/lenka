      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-4xl relative">
          <button
            ...
            className="absolute bottom-4 right-4 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold px-3 py-1.5 rounded-lg transition duration-200 shadow-md z-10"
          >
            Leave
          </button>
          
          <div className="text-center mb-6">
            <h1 className="text-3xl font-bold text-green-600 mb-2">Round {roundIndex + 1} Results</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="px-4 py-3 text-right font-semibold text-gray-700">Guess</th>
              ...
              <tbody>
                {roundResults.results.map((result: any) => (
                  <tr key={result.playerId} className="border-b">
                    <td className="px-4 py-3 font-medium text-gray-800">{result.playerName}</td>
          ...
          <div className="bg-lenka-cream rounded-lg p-4 border-2 border-lenka-mustard/20">
            <h3 className="font-semibold text-gray-700 mb-2">Current Standings</h3>
          ...
          <div className="text-center mt-6">
            {roundIndex + 1 < totalRounds ? (
              <div className="text-gray-600">
                ...
                <p className="text-4xl font-bold text-lenka-red mt-2">{nextRoundCountdown}s</p>
              </div>
            ) : (
              <div className="text-gray-600">
                <p className="text-lg">Final results coming in...</p>
                <p className="text-4xl font-bold text-lenka-red mt-2">{nextRoundCountdown}s</p>
              </div>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl relative">
          ...
          <div className="text-center mb-6">
            <div className="flex items-center justify-center gap-4 mb-2">
              <h1 className="text-3xl font-bold text-lenka-red">
                Round {roundIndex + 1} / {totalRounds}
              </h1>
              <div className={`text-3xl font-bold ${timeLeft <= 5 ? 'text-red-600' : 'text-gray-700'}`}>
                {timeLeft}s
              </div>
            </div>
            {error && (
          ...
          <div className="mb-6 text-center">
            <div className="mb-4 bg-gray-100 rounded-lg p-4 inline-block">
              <Image
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-2">{currentProduct.name}</h2>
            {currentProduct.brand && (
              <p className="text-lenka-mustard font-bold mb-1 text-lg">{currentProduct.brand}</p>
            )}
          ...
          <div className="mb-6">
            <label className="block text-gray-700 font-medium mb-2">Your Guess (€)</label>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent disabled:bg-gray-100 text-lg"
              />
              <button
                onClick={handleSubmitGuess}
                disabled={hasSubmitted || !guess}
                className="bg-lenka-red hover:bg-lenka-red/90 disabled:bg-gray-400 text-white font-semibold px-6 py-3 rounded-lg transition duration-200 border-2 border-lenka-red disabled:border-gray-400"
              >
          ...
          {hasSubmitted && (
            <div className="bg-green-100 border-2 border-green-400 text-green-700 px-4 py-3 rounded text-center">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl">
          <div className="text-center mb-6">
            <h1 className="text-4xl font-bold text-lenka-red mb-2">Game Over!</h1>
          ...
          {currentPlayer?.isHost && (
            <button
              ...
              className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3 px-6 rounded-lg transition duration-200 mb-2"
            >
              Play Again
            </button>
          )}
  ...
  if (lobby.status === 'waiting') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl">
          <div className="text-center mb-6">
            <h1 className="text-4xl font-bold text-lenka-red mb-2">Lenka</h1>
            <div className="flex items-center justify-center gap-2 mb-4">
              <p className="text-2xl font-mono font-bold text-gray-800">{lobby.code}</p>
              <button
                onClick={copyLobbyCode}
                className="text-lenka-red hover:text-lenka-red/80 text-sm underline"
              >
                Copy
              </button>
            </div>
            <p className="text-gray-600">Rounds: {lobby.roundsTotal}</p>
          </div>
          ...
          <div className="mb-6">
            <h2 className="text-xl font-semibold mb-3 text-gray-700">Players ({lobby.players.length})</h2>
            <div className="space-y-2">
              {lobby.players.map((player) => (
                <div
                  key={player.id}
          ...
          {currentPlayer?.isHost ? (
            <button
              ...
              className="w-full bg-lenka-red hover:bg-lenka-red/90 disabled:bg-gray-400 text-white font-semibold py-3 px-6 rounded-lg transition duration-200 mb-2 border-2 border-lenka-red disabled:border-gray-400"
            >
          ...
          <button
            onClick={handleLeaveLobby}
            className="w-full bg-gray-300 hover:bg-gray-400 text-gray-700 font-semibold py-3 px-6 rounded-lg transition duration-200 mt-2"
          >
            Leave Lobby
          </button>
        </div>
